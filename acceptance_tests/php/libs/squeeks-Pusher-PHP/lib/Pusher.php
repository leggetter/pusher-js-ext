...
class Pusher
{
	...
	* @param mixed $encode whether the response should be JSON encoded. Optional, defaults to true
	* @return string
	*/
	public function socket_auth( $channel, $socket_id, $custom_data = false, $encode = true )
	{
		...
		return ($encode? json_encode( $signature ) : $signature);
	}
	...
	* @return string
	*/
	public function presence_auth( $channel, $socket_id, $user_id, $user_info = false, $encode = true )
	{
    ...
    $data = ($encode? json_encode( $user_data ) : $user_data);
		return $this->socket_auth($channel, $socket_id, $data, $encode );
	}
